...
class EventFeatureExtractor:
    ...
    Features:
    - Temporal: is_off_hours, is_weekend, hour_of_day, day_of_week
    ...
    def _calculate_frequency_features(self, current_event: Dict[str, Any], current_dt: Optional[datetime]) -> List[float]:
        ...
        for hist_event in self.event_history:
            ...
            if hist_ts >= window_10m:
                event_type = (hist_event.get('type') or '').lower()
            ...
            if hist_ts >= window_1h:
                ...
                if 'usb' in dst_path or 'removable' in dst_path or any(
                    letter in dst_path for letter in ['f:', 'e:', 'g:', 'h:']
                ):
                    size = hist_event.get('size') or obj.get('size_bytes') or 0
                    usb_bytes += size
                
                # File operations count
                if event_type in ['file_copy', 'file_move', 'file_delete']:
                    file_ops += 1
    ...
    def _extract_quantitative_features(self, event: Dict[str, Any]) -> List[float]:
        ...
        if entropy == 0:
            ...
            content = event.get('clipboard', {}).get('content', '') or event.get('content', '')
            if content:
    ...
    def _extract_contextual_features(self, event: Dict[str, Any]) -> List[float]:
        ...
        dest_app = (event.get('clipboard', {}).get('dest_app', '') or '').lower()
        dest_domain = (event.get('clipboard', {}).get('dest_domain', '') or '').lower()
        dest_window = (event.get('clipboard', {}).get('dest_window_title', '') or '').lower()
        ...
        if 'usb' in dst_path or 'removable' in dst_path or any(letter in dst_path for letter in ['f:', 'e:', 'g:', 'h:']):
            dest_category = 4.0
        # Cloud Sync
        elif any(cloud in dst_path or cloud in dest_domain for cloud in ['onedrive', 'dropbox', 'google drive', 'drive.google.com']):
            dest_category = 3.0
        # Chat App
        elif any(app in dest_app or app in dest_window for app in ['chatgpt', 'discord', 'slack', 'teams', 'zalo']):
            dest_category = 2.0
        # Browser
        elif dest_app in ['chrome.exe', 'msedge.exe', 'firefox.exe', 'brave.exe']:
            dest_category = 1.0
        ...
        if 'clipboard' in event_type:
            ...
            if 'image' in content_type or 'filelist' in content_type:
                ...
                source_type = 1.0
        elif 'network' in event_type or 'upload' in event_type:
            source_type = 3.0
